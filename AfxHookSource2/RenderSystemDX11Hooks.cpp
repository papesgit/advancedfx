...
public:
    CAfxSharedTextureHost() = default;

    ~CAfxSharedTextureHost() {
        Reset();
    }
    ...
    void Reset() {
        ...
        if (m_pNoBlend) { m_pNoBlend->Release(); m_pNoBlend = nullptr; }
        if (m_pNoDepth) { m_pNoDepth->Release(); m_pNoDepth = nullptr; }
        if (m_pRasterizer) { m_pRasterizer->Release(); m_pRasterizer = nullptr; }
        if (m_pSharedRtv) { m_pSharedRtv->Release(); m_pSharedRtv = nullptr; }
        if (m_pSharedTexture) { m_pSharedTexture->Release(); m_pSharedTexture = nullptr; }
        if (m_pResolveTexture) { m_pResolveTexture->Release(); m_pResolveTexture = nullptr; }
    ...
    void Update(ID3D11DeviceContext* pContext, ID3D11Texture2D* pSource) {
        ...
        if (DXGI_FORMAT_R8G8B8A8_UNORM != normalizedSourceFormat && DXGI_FORMAT_B8G8R8A8_UNORM != normalizedSourceFormat) {
            ...
            if (!s_Warned) {
                advancedfx::Warning("AFXWarning: Shared texture host expected DXGI_FORMAT_R8G8B8A8_UNORM (28) input, got %u.\n", srcDesc.Format);
                s_Warned = true;
            }
            device->Release();
            return;
        ...
        if (srcDesc.SampleDesc.Count > 1 && m_pResolveTexture) {
            pContext->ResolveSubresource(m_pResolveTexture, 0, pSource, 0, srcDesc.Format);
            sourceForSampling = m_pResolveTexture;
        }
        ...
        pContext->OMGetBlendState(&oldBlend, oldBlendFactor, &oldSampleMask);

        ID3D11DepthStencilState* oldDepth = nullptr;
        UINT oldStencilRef = 0;
        pContext->OMGetDepthStencilState(&oldDepth, &oldStencilRef);

        ID3D11RasterizerState* oldRS = nullptr;
        pContext->RSGetState(&oldRS);

        UINT oldViewportCount = 1;
        D3D11_VIEWPORT oldViewport;
        pContext->RSGetViewports(&oldViewportCount, &oldViewport);

        pContext->OMSetRenderTargets(1, &m_pSharedRtv, nullptr);
        ...
        if (m_pNoBlend) {
            ...
            pContext->OMSetBlendState(m_pNoBlend, blendFactor, 0xffffffff);
        }
        if (m_pNoDepth) {
            pContext->OMSetDepthStencilState(m_pNoDepth, 0);
        }
        if (m_pRasterizer) {
        ...
        pContext->PSSetShader(m_pCopyPS, nullptr, 0);
        ...
        if (oldBlend) {
            pContext->OMSetBlendState(oldBlend, oldBlendFactor, oldSampleMask);
        }
        if (oldDepth) {
            pContext->OMSetDepthStencilState(oldDepth, oldStencilRef);
        }
        if (oldRS) {
            pContext->RSSetState(oldRS);
        }
        if (oldViewportCount) {
            pContext->RSSetViewports(oldViewportCount, &oldViewport);
        }
...
private:
    static DXGI_FORMAT NormalizeFormat(DXGI_FORMAT fmt) {
        if (fmt == DXGI_FORMAT_R8G8B8A8_TYPELESS) return DXGI_FORMAT_R8G8B8A8_UNORM;
    ...
    bool EnsureResources(ID3D11Device* device, const D3D11_TEXTURE2D_DESC& srcDesc, DXGI_FORMAT normalizedSourceFormat) {
        if (m_pDevice && m_pDevice != device) {
            Reset();
        }
        ...
        if (!m_pFullscreenVS || !m_pCopyPS || !m_pSampler || !m_pNoBlend || !m_pNoDepth || !m_pRasterizer) {
            if (!CreatePipeline()) return false;
        }

        if (srcDesc.SampleDesc.Count > 1) {
            bool needResolve = m_pResolveTexture == nullptr
            ...
            if (needResolve) {
                ...
                m_pDevice->CreateTexture2D(&resolveDesc, nullptr, &m_pResolveTexture);
            }
        } else if (m_pResolveTexture && srcDesc.SampleDesc.Count == 1) {
            m_pResolveTexture->Release();
            m_pResolveTexture = nullptr;
        }
        ...
        if (needsTexture) {
            ...
            rtvDesc.Format = sharedDesc.Format;
            rtvDesc.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;
            rtvDesc.Texture2D.MipSlice = 0;
            if (FAILED(m_pDevice->CreateRenderTargetView(m_pSharedTexture, &rtvDesc, &m_pSharedRtv))) {
        ...
        return m_pSharedTexture && m_pSharedRtv;
    }

    bool CreatePipeline() {
        static const char* s_FullscreenVS = R"(
struct VS_OUTPUT {
...
float4 main(PS_INPUT input) : SV_TARGET {
    return sourceTexture.Sample(sourceSampler, input.uv);
        ...
        HRESULT hr = D3DCompile(s_FullscreenVS, strlen(s_FullscreenVS), "FullscreenVS", nullptr, nullptr, "main", "vs_5_0", D3DCOMPILE_ENABLE_STRICTNESS, 0, &vsBlob, &errorBlob);
        if (FAILED(hr)) {
            advancedfx::Warning("AFXERROR: Failed to compile shared texture VS (0x%x).\n", hr);
        ...
        hr = m_pDevice->CreateVertexShader(vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(), nullptr, &m_pFullscreenVS);
        vsBlob->Release();
        if (FAILED(hr)) {
            advancedfx::Warning("AFXERROR: Failed to create shared texture VS (0x%x).\n", hr);
            return false;
        }

        hr = D3DCompile(s_CopyPS, strlen(s_CopyPS), "CopyPS", nullptr, nullptr, "main", "ps_5_0", D3DCOMPILE_ENABLE_STRICTNESS, 0, &psBlob, &errorBlob);
        if (FAILED(hr)) {
            advancedfx::Warning("AFXERROR: Failed to compile shared texture PS (0x%x).\n", hr);
            if (errorBlob) errorBlob->Release();
            return false;
        }
        if (errorBlob) {
        ...
        hr = m_pDevice->CreatePixelShader(psBlob->GetBufferPointer(), psBlob->GetBufferSize(), nullptr, &m_pCopyPS);
        psBlob->Release();
        if (FAILED(hr)) {
            advancedfx::Warning("AFXERROR: Failed to create shared texture PS (0x%x).\n", hr);
            return false;
        }
        ...
        m_pDevice->CreateBlendState(&blendDesc, &m_pNoBlend);
        ...
        m_pDevice->CreateDepthStencilState(&depthDesc, &m_pNoDepth);
        ...
        m_pDevice->CreateRasterizerState(&rasterDesc, &m_pRasterizer);

        return m_pFullscreenVS && m_pCopyPS && m_pSampler && m_pNoBlend && m_pNoDepth && m_pRasterizer;
    }

    ID3D11Device* m_pDevice = nullptr;
    ID3D11Texture2D* m_pSharedTexture = nullptr;
    ID3D11RenderTargetView* m_pSharedRtv = nullptr;
    ID3D11Texture2D* m_pResolveTexture = nullptr;
    ID3D11SamplerState* m_pSampler = nullptr;
    ID3D11VertexShader* m_pFullscreenVS = nullptr;
    ID3D11PixelShader* m_pCopyPS = nullptr;
    ID3D11BlendState* m_pNoBlend = nullptr;
...
unsigned char * __fastcall New_SceneSystem_CreateRenderContextPtr2(unsigned char *param_1,unsigned char param_2, void* pDevice, const char * fmt, ...) {
    ...
    if (fmt && 0 == strcmp(fmt, "UpdateBuffers")) {
        ...
        if (pszArg0 && 0 == strncmp("Player ", pszArg0, 7)) {
            ...
            if (setupSerial) {
                if (void* pCRenderContextDx11_SoftwareCommandList = *(void**)param_1) {
                    auto fnQueueCallback = (void(__fastcall*)(void* pCRenderContextDx11_SoftwareCommandList, void* pCallback))(*(void***)pCRenderContextDx11_SoftwareCommandList)[142];
