#pragma once

#include "FreecamController.h"
#include "ObsObserverState.h"

#include <string>

struct FreecamConfigDelta {
	bool hasMouseSensitivity = false;
	float mouseSensitivity = 0.0f;

	bool hasMoveSpeed = false;
	float moveSpeed = 0.0f;
	bool hasSprintMultiplier = false;
	float sprintMultiplier = 0.0f;
	bool hasVerticalSpeed = false;
	float verticalSpeed = 0.0f;
	bool hasSpeedAdjustRate = false;
	float speedAdjustRate = 0.0f;
	bool hasSpeedMinMultiplier = false;
	float speedMinMultiplier = 0.0f;
	bool hasSpeedMaxMultiplier = false;
	float speedMaxMultiplier = 0.0f;

	bool hasRollSpeed = false;
	float rollSpeed = 0.0f;
	bool hasRollSmoothing = false;
	float rollSmoothing = 0.0f;
	bool hasLeanStrength = false;
	float leanStrength = 0.0f;
	bool hasLeanAccelScale = false;
	float leanAccelScale = 0.0f;
	bool hasLeanVelocityScale = false;
	float leanVelocityScale = 0.0f;
	bool hasLeanMaxAngle = false;
	float leanMaxAngle = 0.0f;
	bool hasLeanHalfTime = false;
	float leanHalfTime = 0.0f;
	bool hasClampPitch = false;
	bool clampPitch = false;

	bool hasFovMin = false;
	float fovMin = 0.0f;
	bool hasFovMax = false;
	float fovMax = 0.0f;
	bool hasFovStep = false;
	float fovStep = 0.0f;
	bool hasDefaultFov = false;
	float defaultFov = 0.0f;

	bool hasSmoothEnabled = false;
	bool smoothEnabled = false;
	bool hasHalfVec = false;
	float halfVec = 0.0f;
	bool hasHalfRot = false;
	float halfRot = 0.0f;
	bool hasLockHalfRot = false;
	float lockHalfRot = 0.0f;
	bool hasLockHalfRotTransition = false;
	float lockHalfRotTransition = 0.0f;
	bool hasHalfFov = false;
	float halfFov = 0.0f;
	bool hasRotCriticalDamping = false;
	bool rotCriticalDamping = false;
	bool hasRotDampingRatio = false;
	float rotDampingRatio = 0.0f;

	bool hasWalkMoveSpeed = false;
	float walkMoveSpeed = 0.0f;
	bool hasWalkMoveAcceleration = false;
	float walkMoveAcceleration = 0.0f;
	bool hasWalkMoveDeceleration = false;
	float walkMoveDeceleration = 0.0f;
	bool hasWalkRunMultiplier = false;
	float walkRunMultiplier = 0.0f;
	bool hasWalkCrouchSpeedMultiplier = false;
	float walkCrouchSpeedMultiplier = 0.0f;
	bool hasWalkLookHalfTime = false;
	float walkLookHalfTime = 0.0f;
	bool hasWalkFovHalfTime = false;
	float walkFovHalfTime = 0.0f;
	bool hasWalkGravity = false;
	float walkGravity = 0.0f;
	bool hasWalkJumpSpeed = false;
	float walkJumpSpeed = 0.0f;
	bool hasWalkHullRadius = false;
	float walkHullRadius = 0.0f;
	bool hasWalkHullHalfHeight = false;
	float walkHullHalfHeight = 0.0f;
	bool hasWalkCrouchHullHalfHeight = false;
	float walkCrouchHullHalfHeight = 0.0f;
	bool hasWalkCameraTopInset = false;
	float walkCameraTopInset = 0.0f;
	bool hasWalkStepHeight = false;
	float walkStepHeight = 0.0f;
	bool hasWalkGroundProbe = false;
	float walkGroundProbe = 0.0f;
	bool hasWalkMinGroundNormalZ = false;
	float walkMinGroundNormalZ = 0.0f;
	bool hasWalkTraceMask = false;
	uint32_t walkTraceMask = 0;
	bool hasWalkModeDefaultEnabled = false;
	bool walkModeDefaultEnabled = false;
	bool hasHandheldDefaultEnabled = false;
	bool handheldDefaultEnabled = false;
	bool hasWalkBobAmplitudeZ = false;
	float walkBobAmplitudeZ = 0.0f;
	bool hasWalkBobAmplitudeSide = false;
	float walkBobAmplitudeSide = 0.0f;
	bool hasWalkBobAmplitudeRoll = false;
	float walkBobAmplitudeRoll = 0.0f;
	bool hasWalkBobFrequency = false;
	float walkBobFrequency = 0.0f;
	bool hasHandheldShakePosAmplitude = false;
	float handheldShakePosAmplitude = 0.0f;
	bool hasHandheldShakeAngAmplitude = false;
	float handheldShakeAngAmplitude = 0.0f;
	bool hasHandheldShakeFrequency = false;
	float handheldShakeFrequency = 0.0f;
	bool hasHandheldDriftPosAmplitude = false;
	float handheldDriftPosAmplitude = 0.0f;
	bool hasHandheldDriftAngAmplitude = false;
	float handheldDriftAngAmplitude = 0.0f;
	bool hasHandheldDriftFrequency = false;
	float handheldDriftFrequency = 0.0f;
};

struct FreecamHandoffPayload {
	CameraTransform transform;
	bool hasFov = false;
	float fov = 90.0f;
	bool hasSmoothTransform = false;
	CameraTransform smoothTransform;
	bool hasSmoothQuat = false;
	Afx::Math::Quaternion smoothQuat;
	bool hasSmoothFov = false;
	float smoothFov = 90.0f;
	bool hasSpeedScalar = false;
	float speedScalar = 1.0f;
	bool hasWalkRuntimeState = false;
	WalkCameraRuntimeState walkRuntimeState;
	FreecamConfigDelta configDelta;
	std::string message;
};

struct CameraTransformSamples {
	CameraTransform previous;
	CameraTransform current;
	float deltaTime = 0.0f;
	bool hasPrevious = false;
};

enum class FreecamInitMode {
	InheritMotion,
	Static
};

enum class FreecamHoldMode {
	Camera,
	World
};

void ObsWebSocket_QueueFreecamEnable(FreecamInitMode initMode = FreecamInitMode::InheritMotion);
void ObsWebSocket_QueueFreecamDisable();
void ObsWebSocket_QueueFreecamHold(bool hasMode, FreecamHoldMode mode);
void ObsWebSocket_QueueFreecamConfig(const FreecamConfigDelta& delta, const std::string& message);
void ObsWebSocket_QueueFreecamHandoff(const FreecamHandoffPayload& payload);
void ObsWebSocket_QueueAttachCamera(const AttachmentCameraState& state);
void ObsWebSocket_QueueRefreshBinds();
void ObsWebSocket_QueueSetAltSpectatorBindings(bool enabled);
void ObsWebSocket_QueueExecCommand(const std::string& cmd);
void ObsWebSocket_QueueCampathPlay(const std::string& cmd, double offset);
void ObsWebSocket_ProcessActions();

CameraTransform Obs_GetLastCameraTransform();
CameraTransformSamples Obs_GetRecentCameraTransforms();
