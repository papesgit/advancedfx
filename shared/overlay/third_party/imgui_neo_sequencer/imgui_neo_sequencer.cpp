...
namespace ImGui
{
    ...
    static bool createKeyframe(int32_t* frame)
    {
        ...
        // Single-click select support: if selection enabled and user clicks a hovered keyframe,
        // add it to selection so dragging can start immediately without box-select.
        if (context.SelectionEnabled && hovered && IsMouseClicked(ImGuiMouseButton_Left) &&
            (context.StateOfSelection != SelectionState::Selecting) && !context.Selection.contains(id))
        {
            const ImGuiID timelineId = context.TimelineStack.back();
            addKeyframeToDeleteData(*frame, context, timelineId);
            context.Selection.push_back(id);
        }
