...
namespace ImGui {
    ...
    RenderNeoSequencerTopBarOverlay(float zoom, float valuesWidth,uint32_t startFrame, uint32_t endFrame, uint32_t offsetFrame, const ImVec2 &cursor, const ImVec2 &size,
        ...
        if(drawFrameLines) {
            ...
            // Determine major/minor tick spacing so that major ticks are at least maxPixelsPerTick apart
            float perFrameWidthOrig = GetPerFrameWidth(size.x, valuesWidth, endFrame, startFrame, zoom);
            if(perFrameWidthOrig <= 0.0f) return;

            uint32_t primaryFrames = 1;    // minor tick step in frames
            uint32_t secondaryFrames = 10; // major tick step in frames

            while (perFrameWidthOrig * primaryFrames < maxPixelsPerTick)
            {
                primaryFrames *= 10;
                secondaryFrames *= 10;
            }
            ...
            for(int32_t i = 0; i < count; i++) {
                ...
                const bool primaryFrame = (frame % primaryFrames) == 0;
                const bool secondaryFrame = (frame % secondaryFrames) == 0;
                if(!primaryFrame && !secondaryFrame) continue;

                const float lineHeight = secondaryFrame ? barArea.GetSize().y : barArea.GetSize().y * 0.5f;
                const float x = barArea.Min.x + (float)i * perFrameWidthOrig;
                ...
                if(drawFrameText && secondaryFrame) {
                    char text[16];
                    const int printRes = snprintf(text, sizeof(text), "%u", frame);
                    if(printRes > 0) {
